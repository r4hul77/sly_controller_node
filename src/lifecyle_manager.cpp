...
public:
  ...
  : Node(node_name)
  {
    this->declare_parameter("lifecycle_node_to_manage", "imu_node");
    this->node_to_manage = this->get_parameter("lifecycle_node_to_manage").as_string();
    node_get_state_topic  =  this->node_to_manage + "/get_state";
    node_change_state_topic = this->node_to_manage + "/change_state";
  }
  ...
  get_state(std::chrono::seconds time_out = 3s)
  {
    ...
    if (future_status != std::future_status::ready) {
      RCLCPP_ERROR(
        get_logger(), "Server time out while getting current state for node %s", node_to_manage.c_str());
      return lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
    }
    ...
    if (future_result.get()) {
      RCLCPP_INFO(
        ...
        node_to_manage.c_str(), future_result.get()->current_state.label.c_str());
      return future_result.get()->current_state.id;
    } else {
      RCLCPP_ERROR(
        get_logger(), "Failed to get current state for node %s", node_to_manage.c_str());
      return lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN;
    }
  }
